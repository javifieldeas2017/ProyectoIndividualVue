<template>
    <section id="detail" class="col-xs-7 col-sm-7">
        ...
        <div class="form-group">
            ...
            <input name="pelicula" type="text" v-model="selected.Pelicula" class="form-control" placeholder="Introduce película" @input="validateEntrada($event)" @keyup="validateEntrada($event)" title="Escribe un título de 20 caracteres">
        </div>
        <div class="form-group">
            <label for="fila">Fila</label>
            <input name="fila" type="number" v-model="selected.Fila" class="form-control" @input="validateEntrada($event)" @keyup="validateEntrada($event)" placeholder="Introduce fila">
        </div>
        <div class="form-group">
            <label for="butaca">Butaca</label>
            <input name="butaca" type="number" v-model="selected.Butaca" class="form-control" @input="validateEntrada($event)" @keyup="validateEntrada($event)" placeholder="Introduce butaca">
        </div>
        <div class="form-group">
            <label for="precio">Precio (€)</label>
            <input name="precio" type="number" step="0.01" v-model="selected.Precio" class="form-control" @input="validateEntrada($event)" @keyup="validateEntrada($event)" placeholder="Introduce precio">
        </div>
        <div class="checkbox">
            <label><input name="online" type="checkbox" v-model="selected.Online" @input="validateEntrada($event)">Online</label>
        </div>
